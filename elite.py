...
try:
    with open('styles.css', 'r') as f:
        ...
        st.markdown(f'<style>/* v20260820i */ {css_content}</style>', unsafe_allow_html=True)
        ...
        if equipos_unicos:
            ...
            for tid, equipo in list(equipos_por_procesar.items())[:total_equipos_a_procesar]:
                ...
                # ★ CASO B: EQUIPO EXISTENTE (ya tiene records en DB)
                    ...
                    # ★ FIX: buscar si historial < 5 (no <3) para llenarlo
                    # Si historial >= 5: solo agregar el FT específico de la ventana
                    ...
                    if not ft_en_ventana and not historial_corto:
                        ...
                        logger.info(f"Historial <5 para {team_name}: trae API hasta 5 FT")
                    elif ft_en_ventana and not historial_corto:
                        pass  # historial completo y hay FT pendiente -> guardar solo ese 1

                    # ★ ACUMULACIÓN: traer últimos 5 FT desde la API
                    # excluir_fixture_ids evita re-descargar stats de los ya guardados.
                    try:
                        # max_partidos=5 (no 5) + descanso para no matar Render
...
def render_vip_page():
    ...
    with tab_bankroll:
        ...
        with sub_tab1:
            ...
            if apuestas:
                ...
                # Bankroll grande
                color_ganancia = "#4ade80" if ganancia_efectiva >= 0 else "#f87171"
                color_label = "#cbd5e1"
                st.markdown(f"""
                <div style="background: linear-gradient(135deg, #1a3a2a 0%, #0f2518 100%); 
                    ...
                    <div style="color: {color_ganancia}; font-size: 1.2rem; font-weight: 600;">{'+' if ganancia_efectiva >= 0 else ''}{format_money(ganancia_efectiva, simbolo)} ({'+' if roi >= 0 else ''}{roi:.1f}%)</div>
                </div>
                """, unsafe_allow_html=True)
                ...
                elif bankroll_actual >= bankroll_guardado * 1.1:
        ...
        # ========== SUBTAB 2: AGREGAR ==========
        with sub_tab2:
            # Pendiente = sin resultado registrado (mismo criterio que la
            # sync: resultado_1x2 IS NULL). No usar acertado_1x2: los picks
    ...
    with sub_tab4:
        ...
        # Todo en una fila: Moneda + Accion + Monto + Ejecutar (pedido por
        # el usuario para ahorrar espacio vertical). 'Inicial' ya no existe:
        # solo se suma (Depositar) o resta (Retirar) con validacion.
        st.markdown("""
        <style>
        /* Labels del form legibles (no diminutos) */
        ...
        accion = "💵 Depositar"  # valor por defecto (form lo sobreescribe)
        monto = 0.0
        ejecutar = False
        with st.form("form_cash", clear_on_submit=True):
            col_mon, col_acc, col_monto, col_btn = st.columns([1.5, 2, 2, 1.2])
            with col_mon:
            ...
            with col_acc:
                accion = st.selectbox("Acción", ["💵 Depositar", "💸 Retirar"], key="accion_cash")
            with col_monto:
                monto = st.number_input("Monto", min_value=0.0, step=10.0, key="monto_cash")
            with col_btn:
